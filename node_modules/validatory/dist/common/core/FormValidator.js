...
class FormValidator {
  ...
  onDomMutated({ addedNodes, removedNodes }) {
    this.formElements = this.formElements.filter(formElement => removedNodes.find(removedNode => removedNode === formElement.formElementDomNode) === undefined);
  ...
  preProcessFormElementValidationPattern(formElementDomNode) {
    const builtInValidationPatternsKeys = ['email', 'phone', 'any'],
          needsPreProcessing = builtInValidationPatternsKeys.some(key => formElementDomNode.hasAttribute(`data-validate-${key}`));
  ...
  isValid() {
    ...
    return new Promise(resolve => validPromises.then(validResults => resolve(validResults.every(validResult => validResult.valid))));
  }
